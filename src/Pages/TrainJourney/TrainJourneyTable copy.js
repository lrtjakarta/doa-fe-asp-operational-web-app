...
import { ProfileContext, IncidentContext, UserProfilContext } from "../../Context/index";
...
export default function MenuTablet(props) {
  ...
  const { getUserProfilById, userProfile, operational_id } = useContext(UserProfilContext);
  ...
  const fectData = async () => {
    setLoading(true);
    let dataresponse = await getDailyWorkById(localStorage.operational_id);
    console.log("datarespon", dataresponse);
    if (dataresponse?.data?.loopRouteTrain) {
      if(dataresponse?.data?.trainJourneyLog?.stateJourney === "REST"){
        setRESTState(true)
      }
      let trainStart = dataresponse?.data?.loopRouteTrain?.route.filter((x) => x.status === "Start")[0];
  ...
  const TrainStartClick = async () => {
    ...
    if(RESTState) // Jika kondisi Turun Kabin
    {
      await updateDataScheduleTrainDriverById(localStorage.operational_id, {
    ...
    if (nextStep) { //Cek apakah akhir perjalanan sdh stasiun akhir
    ...
    if ( //Cek apakah routenya belum selesai
      dataDetailDaily?.loopRouteTrain?.route.filter((x) => x.status !== "Finish")[0]?.station.filter((x) => x.status === "").length ==
      dataDetailDaily?.loopRouteTrain?.route.filter((x) => x.status !== "Finish")[0]?.station.length
    ) {
      let datapost = {
        _id: dataDetailDaily?._id,
        loopRouteTrainId: dataDetailDaily?.loopRouteTrain?._id,
        profile : {_id : userProfile?.officerId, name: userProfile?.officerName},
        trainNumber: dataDetailDaily?.loopRouteTrain?.route.filter((x) => x.status !== "Finish"
        )[0]?.trainNumber,
        startTime: time,
    ...
    if (dataDetailDaily?.loopRouteTrain?.route.filter((x) => x.status !== "Finish")[0]?.station.filter((x) => x.status === "Finish").length > 0
    ) {
      ...
      datapost = {
        ...
        loopRouteTrainId: dataDetailDaily?.loopRouteTrain?._id,
        profile : {_id : userProfile?.officerId, name: userProfile?.officerName},
        stationId: !RESTState ? dataDetailDaily?.loopRouteTrain?.route.filter((x) => x.status !== "Finish")[0]?.station.filter((x) => x.status === "")[0]._id : "",
        trainNumber: !RESTState ? dataDetailDaily?.loopRouteTrain?.route.filter((x) => x.status !== "Finish")[0]?.trainNumber : "",
        startTime: time,
        dweelingTime: dweelingTime,
      };
  ...
  const TrainFinishClick = async () => {
    ...
    let datapostLast = {
      ...
      loopRouteTrainId: dataDetailDaily?.loopRouteTrain?._id,
      profile : {_id : userProfile?.officerId, name: userProfile?.officerName},
      stationId: dataDetailDaily?.loopRouteTrain?.route.filter((x) => x.status !== "Finish")[0]?.station.filter((x) => x.status !== "Finish")[0]?._id,
      trainNumber: dataDetailDaily?.loopRouteTrain?.route.filter((x) => x.status !== "Finish")[0]?.trainNumber,
      status: "Finish",
    };
    console.log("datapost", datapost);
    ...
    if (datapost.stationId === "62d115995be964c7a53d990c" ||datapost.stationId === "62d1163e5be964c7a53d991b" || RESTState) {
    ...
    if (
      ...
      if (dataDetailDaily?.kaIncident.length > 0) {
        ...
        if (response.status) {
          ...
          await getDailyWorkById(localStorage.operational_id);
  ...
  return (
    <div style={{ flex: 1, height: window.innerHeight - 65 }}>
      ...
      <DialogCustom
          ...
          <div>
            {incident.map((item, index) => (
              <div style={{ marginTop: 5 }}>
                ...
                <TextField
                  style={{ width: "100%" }}
                  placeholder={"Masukkan " + item.title + " disini"}
                  multiline={true}
                  value={item.value}
                  onChange={(e) => {
                    const datatemp = incident.map((x, i) => {
                      if (i === index) {
                        x.value = e.target.value;
                        x.createdAt = moment().toISOString();
                      }
                      return x;
                    });
                    setIncident(datatemp);
                  }}
                />
              </div>
            ))}
                ...
                <Stack
                  ...
                  {videoSrc?.map((item, index) => (
                    <Box>
                      ...
                      <Button
                        onClick={() => handleOpenDialogVideoDelete(item)}
                        fullWidth
                        sx={{
                          backgroundColor: "red",
                          color: "white",
        ...
        confirm={async () => {
          let sendIncident = [
            ...
            {
              title: typeMedia,
              file: typeMedia === "image" ? multiImage : videoSrc,
      ...
      {loading ? (
      ...
      ) : dataDetailDaily?.readTakeGive ||
        (
          (dataDetailDaily?.loopRouteTrain?.code === "A" ||
          dataDetailDaily?.loopRouteTrain?.code === "B" ||
          dataDetailDaily?.loopRouteTrain?.code === "C" ||
          dataDetailDaily?.loopRouteTrain?.code === "D") 
          // &&
          // gonoGoItem.length > 0 
        ) 
          ? 
            ...
            <Paper>
              <Grid container spacing="2" sx={{ mt: 1 }}>
                ...
                <Grid item xs="12" sm="12" md="12">
                  {
                    ...
                    <Paper style={{padding:20}}>
                      <Typography variant="h4" style={{textAlign:'center', marginBottom:10, marginTop:50}}>Silahkan Rehat sejenak untuk melanjutkan ke Perjalanan Nomor KA berikutnya</Typography>
                      <center>
                                ...
                                <Hidden only={["xl", "lg", "sm", "md"]}>
                                  <TableCell
                                    ...
                                    sx={{
                                      ...
                                      <Typography style={{fontSize:12}}>TIBA : {itemStation.endPlan|| "xx:xx:xx"} | {itemStation.end|| "xx:xx:xx"}</Typography> 
                                      <Typography style={{fontSize:12}}>BERANGKAT : {itemStation.startPlan|| "xx:xx:xx"} | {itemStation.start || "xx:xx:xx"}</Typography>
                                      <Typography style={{fontSize:12}}>DWELLING : {itemStation?.dweelingTime > 0
                                    ? moment()
                                        .startOf("day")
                                        .seconds(itemStation.dweelingTime)
                                        .format("H:mm:ss")
                ...
                {!RESTState && <Button
                    ...
                    onClick={async () => {
                       const KANext = dataDetailDaily?.loopRouteTrain?.route.filter((x) => x.status !== "Finish")[1]?.trainNumber
                       if(KANext === "REST"){
                        setRESTState(true)
                       }
                       else{
        ...
        <Container maxWidth="xl" sx={{ mt: 13 }}>
          <Paper sx={{ p: 2 }}>
            {JSON.stringify(dataDetailDaily.loopRouteTrain.code)}
            {dataDetailDaily?.loopRouteTrain?.code === "A" ||
            dataDetailDaily?.loopRouteTrain?.code === "B" ||
            dataDetailDaily?.loopRouteTrain?.code === "C" ||
            dataDetailDaily?.loopRouteTrain?.code === "D" ? (
              <Typography variant="h4" align="center">
                Silahkan membuat Data Go No Item terlebih dahulu
              </Typography>
            ) : (
              <Typography variant="h4" align="center">
                Silahkan menyelesaikan serah terima dinas dengan masinis
                sebelumnya
              </Typography>
            )}
          </Paper>
