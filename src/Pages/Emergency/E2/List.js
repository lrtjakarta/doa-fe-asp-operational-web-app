...
function List() {
	...
	// context
	const { listEmergency2, getEmergency2 } = useContext(EmergencyContext);
	...
	const handleDeleteData = async () => {
		const respon = await ApiOperational.deleteEmergency2(dataRow._id);
		if (respon) {
			...
			<Dialog open={dialogPrint} fullScreen>
				<Container maxWidth="xl">
					...
					<div ref={componentRef}>
						<Box sx={{ p: 3 }}>
							<HeaderPrintV2
								title1="Emergency Form E.2"
								title2="Perintah Berjalan Hati - Hati"
								number="LRTJ-FR-POP-007-b"
								revisi="01"
								page="Page 3 of 4"
							/>
							<BodyPrint dataId={dataRow?.row} dataUser={dataRow?.user} />
						</Box>
					</div>
				</Container>
			</Dialog>
