...
function Form() {
	...
	const handleSubmit = async () => {
		...
		if (type === 'Edit') {
			try {
				const respon = await ApiOperational.updateEmergency7(
					dataRow?._id,
					postData
				);
			...
			try {
				const respon = await ApiOperational.postEmergency7(postData);
				// console.log("respon", respon);
				if (respon.statusText === 'OK') {
					resetForm();
					history.goBack(); // Mengirim respon ke parent (List)
	...
	// useEffect
	useEffect(() => {
		getMasterEmergency({ title: ['Emergency 7'] });
		if (type === 'Edit') {
			getDataById();
		}
	}, []);
	...
	useEffect(() => {
		const _footer = listMasterEmergency.filter(x => x.title === 'Emergency 7');
				...
				<Grid container spacing={2}>
					...
					<Grid item xs={12} sm={3}>
						<AppTextField
							fullWidth
							size="small"
							label="No. Urut dibuku Dispatcher/JB/FS"
							sx={{ mt: '0px' }}
