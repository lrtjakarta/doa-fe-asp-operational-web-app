...
function Form() {
	...
	const handleSubmit = async () => {
		...
		if (type === 'Edit') {
			try {
				const respon = await ApiOperational.updateEmergency6(
					dataRow?._id,
					postData
				);
			...
			try {
				const respon = await ApiOperational.postEmergency6(postData);
				// console.log("respon", respon);
				if (respon.statusText === 'OK') {
					resetForm();
					history.goBack(); // Mengirim respon ke parent (List)
	...
	// useEffect
	useEffect(() => {
		getMasterEmergency({ title: ['Emergency 6', 'All'] });
		if (type === 'Edit') {
			getDataById();
		}
	}, []);
