...
function List() {
	...
	// context
	const { listEmergency1, getEmergency1 } = useContext(EmergencyContext);
	const { userProfile } = useContext(UserProfilContext);
	// console.log('userById', userProfile);
	...
	const handleDeleteData = async () => {
		const respon = await ApiOperational.deleteEmergency1(dataRow._id);
		if (respon) {
			...
			<Dialog open={dialogPrint} fullScreen>
				<Container maxWidth="xl">
					...
					<div ref={componentRef}>
						<Box sx={{ p: 3 }}>
							<HeaderPrintV2
								title1="Emergency Form E.1"
								title2="Perintah Melewati Sinyal Utama"
								title3="Indikasi Tidak Aman"
								number="LRTJ-FR-POP-007-a"
								revisi="01"
								page="Page 3 of 3"
							/>
