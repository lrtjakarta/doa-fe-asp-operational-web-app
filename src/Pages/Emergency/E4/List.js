...
function List() {
	...
	// context
	const { listEmergency4, getEmergency4 } = useContext(EmergencyContext);
	...
	const handleDeleteData = async () => {
		const respon = await ApiOperational.deleteEmergency4(dataRow._id);
		if (respon) {
			...
			<Dialog open={dialogPrint} fullScreen>
				<Container maxWidth="xl">
					...
					<div ref={componentRef}>
						<Box sx={{ p: 3 }}>
							<HeaderPrintV2
								title1="Emergency Form E.4"
								title2="Perintah Umum"
								number="LRTJ-FR-POP-007-d"
								revisi="01"
								page="Page 3 of 3"
							/>
