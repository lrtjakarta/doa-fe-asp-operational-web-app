...
function Form() {
	...
	const handleSubmit = async () => {
		...
		if (type === 'Edit') {
			try {
				const respon = await ApiOperational.updateEmergency3(
					dataRow?._id,
					postData
				);
			...
			try {
				const respon = await ApiOperational.postEmergency3(postData);
				// console.log("respon", respon);
				if (respon.statusText === 'OK') {
					resetForm();
					history.goBack(); // Mengirim respon ke parent (List)
	...
	const getDataById = () => {
		...
		setFormInfo4(dataRow?.information?.name4);
	};

	// useEffect
	useEffect(() => {
		getMasterEmergency({ title: ['Emergency 3', 'All'] });
		if (type === 'Edit') {
			getDataById();
		}
	}, []);
	...
	useEffect(() => {
		if (type !== 'Edit') {
			...
			const _info3 = dataFilter.filter(x => x.idNumber === 3);
			setFormInfo3(_info3[0]);
			const _info4 = dataFilter.filter(x => x.idNumber === 4);
		...
		<Box sx={{ p: 3, backgroundColor: '#fff', minHeight: '100vh' }}>
			<Container maxWidth="xl">
				<Grid container spacing={2}>
					...
					<Grid item xs={12} sm={12}>
						<QuillEditor
							value={formInfo4?.information}
							onChange={e => handleChange('info4', e)}
						/>
					</Grid>
