...
import { PPPKAContext } from 'Context';
import HeaderPrint from './PrintDoc/HeaderPrint';
import BodayPrint from './PrintDoc/BodayPrint';
...
export default function PPPKAList() {
		...
		<>
			...
			>
				<DialogContent sx={{ width: 600 }}>
					<Typography>
						Apakah anda yakin menghapus dengan PPPKA Tgl:{' '}
						{selectedData?.PPPKADate}
					</Typography>
				</DialogContent>
				<DialogActions sx={{ m: 1 }}>
					...
					<Button
						...
						onClick={() => {
							deleteDataPPPKA(selectedData._id);
							handleClose();
						}}
					>
						Hapus
			...
			>
				<Typography sx={{ color: '#fff', fontSize: 15 }}>
					PPPKA (Penetapan, Pengumuman dan Pembatalan Perjalanan Kereta API)
				</Typography>
			</div>
				...
				<Grid container justifyContent={'flex-end'}>
					...
					<div style={{ marginRight: 10 }}>
						<Button
							...
							onClick={async () => {
								if (date) {
									getDataPPPKA({
										PPPKADate: moment(date).format('YYYY-MM-DD'),
									});
								} else {
									getDataPPPKA();
								}
					...
					<div>
						<Grid container justifyContent={'flex-end'} alignItems="center">
							<Button
								onClick={async () => {
									history.push('/app/operational/pppka/form?action=add');
								}}
								variant="contained"
								...
								dataPPPKA.map((row, index) => {
									return (
										<StyledTableRow key={row.name}>
											...
											<StyledTableCell align="left">
												<Typography
													dangerouslySetInnerHTML={{ __html: row.PPPKANote }}
												/>
											</StyledTableCell>
											<StyledTableCell align="center">
												<IconButton
												...
												<IconButton
													...
													onClick={() => {
														history.push(
															'/app/operational/pppka/form?action=edit&id=' +
																row._id
														);
													}}
												>
			...
			<Dialog fullScreen open={dialogPrint}>
				...
				<div ref={componentRef} style={{ padding: '30px' }}>
					<HeaderPrint
						title="PPPKA (Penetapan, Pengumuman dan Pembatalan Perjalanan Kereta Api)"
						number="LRTJ-FR-POP-001"
						revisi="0"
						page="Page 4 of 4"
					/>
					<BodayPrint dataId={rowData} />
