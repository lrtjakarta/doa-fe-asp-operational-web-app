...
function BodyPrintV2({ dataId }) {
	moment.locale('id');

	const _information = dataId?.information;
	const _lrv = _information?.vehicleNumber;
	const _cabinRide = _information?.cabinRide;

	const totalValue = _cabinRide?.reduce((total, item) => {
	...
	const totalrealization = _cabinRide?.reduce((total, item) => {
		const desc = item.desc ? item.desc : item.description;
		const sumDescValues = desc?.reduce(
	...
	return (
		<Box sx={{ marginTop: '5px' }}>
			<Grid container spacing={0} sx={{ mb: '5px' }}>
				...
				<Grid item xs={12} sm={4}>
					<Typography14 title={':' + ' ' + _lrv?.numberLrv} />
				</Grid>
				...
				<Grid item xs={12} sm={4}>
					<Typography14 title={':' + ' ' + _information?.kaService} />
				</Grid>
				...
				<Grid item xs={12} sm={4}>
					<Typography14 title={':' + ' ' + _information?.officerStart} />
				</Grid>
				...
				<Grid item xs={12} sm={4}>
					<Typography14 title={':' + ' ' + _information?.officerEnd} />
				</Grid>
			</Grid>
						...
						<tr>
							<td>
								<Box sx={{ minHeight: '50px' }}>
									<Typography14 title={_information?.note} />
								</Box>
							</td>
						</tr>
					</tbody>
